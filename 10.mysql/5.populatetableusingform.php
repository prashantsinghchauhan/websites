...
if($_POST["submit"]){
 //check for errors
    ...
    if($errors){
        ...
        $tblname = "users";
        //mysqli_real_escape_string= it removes the special character from the string, data enter krwane se pehle asey hume variable ko tayiayar karna hota hai yeh function laga ke.
        $firstname = mysqli_real_escape_string($link, $firstname);
        $lastname = mysqli_real_escape_string($link, $lastname);
        $email = mysqli_real_escape_string($link, $email);
        $password = mysqli_real_escape_string($link, $password);
        $password = md5($password);//md5 encode karega password ko 32 charcter mai hexadecima ki form mai
        // ek or important baat password ki length dete wakt yaad rakho ki uski lenghth 32 se jyada ho nahi toh password store ni hoga, because md5() function hi 32 lenght le lega fir error ayega so dyan rakhna hai is chij ka.
        
        //execute insert query
        if(!$id){
            //agar user id ni insert karega toh bhi yeh chalega directly kyunki humne table create krte wakt auto increment lagaya tha, toh user na bhi kare id insert toh chalega.
            $sql = "INSERT INTO users (firstname, lastname, email, password) VALUES ('$firstname', '$lastname', '$email', '$password')";   
        }else{
         // or agar kare id entery toh yeh chalega.
            $sql = "INSERT INTO users (ID, firstname, lastname, email, password) VALUES ('$id', '$firstname', '$lastname', '$email', '$password')";   
        }
        //mysqli_query = then query execute karni hai sahi hai toh if statement chal jayegi nahi toh else.
